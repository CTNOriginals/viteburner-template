...
export class ThemeProfile {
	...
	*/
	...
	public applyColorSyntax(input: string): string {
		//TODO bug Fix: 
...
export const defaultThemeProfile = new ThemeProfile({
	...
	overrides: [
		...
		// 	/(\[)(?:\]|.)*?(\])/g,
		// ], new Theme('#aaaaaa')),
		new ThemeOverride(/--\w+/g, new Theme('#ffefa7')),
		new ThemeOverride([
			' + ', ' - ', ' / ', ' * ', ' = ', ' % '
		...
		new ThemeOverride([/.*\(.*\)\s*{/gi], new Theme('#eed06f')),
		new ThemeOverride([
			/new .*?\(.*\)/gi,
			/class .*?\s*{/gi
		], new Theme('#4EC9B0')),
		new ThemeOverride([
			'function', 'async', 'new ', 'class ', 'constructor', 'const ', 'null'
		], new Theme('#0073D1')),
		new ThemeOverride([
			'this', ' in ', ' of ', 'let ', 'var '
		], new Theme('#569CD6')),
		new ThemeOverride([
			'export', 'return', 'if ', 'else ', 'try '
		], new Theme('#C26ABA')),
		new ThemeOverride([
			'for ', 'while', 'break', 'continue', 'case ', ' catch', 'import ', 'from ', ' as '
		], new Theme('#ca4caa')),
		new ThemeOverride([/\/\/.*$/gm], new Theme('#6A9955')),
		...
		new ThemeOverride([/error/gi], new Theme('#ff0000')),
		new ThemeOverride([/ERROR/g, /danger/gi], new Theme('#be0000')),
		new ThemeOverride([/warn(ing)?/gi], new Theme('#ffbb00')),
		new ThemeOverride([/alert/gi], new Theme('#ffbb00')),
